	...
	public boolean checkGamemode(Entity _ent){
		if(_ent instanceof ServerPlayerEntity) {
			...
			return Minecraft.getInstance().getConnection().getPlayerInfo(((AbstractClientPlayerEntity) _ent).getGameProfile().getId()) != null
				&& Minecraft.getInstance().getConnection().getPlayerInfo(((AbstractClientPlayerEntity) _ent).getGameProfile().getId()).getGameType() == eMode() == GameType.${generator.map(field$gamemode, "gamemodes")};
		}
		return false;
	}
