private static String executeCommandGetResult(IWorld world, Vec3d pos, String command) {
	...
	if (world.getWorld() instanceof ServerWorld) {
		...
		((ServerWorld) world.getWorld()).getServer().getCommandManager().handleCommand(new CommandSource(dataConsumer, pos, Vec2f.ZERO, (ServerWorld) world.getWorld(), 4, "", new StringTextComponent(""), ((ServerWorld) world.getWorld()).getServer(), null), command);
	}
