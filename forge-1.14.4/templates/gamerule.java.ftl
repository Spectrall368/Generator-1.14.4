...
public class ${name}GameRule extends ${JavaModName}Elements.ModElement {
	...
	<#if data.type == "Number">
		public static GameRules.RuleType<GameRules.IntegerValue> create(int defaultValue) {
			try {
				Method createGameruleMethod = ObfuscationReflectionHelper.findMethod(GameRules.IntegerValue.class, "func_82764_b", int.class);
	...
	<#else>
		public static GameRules.RuleType<GameRules.BooleanValue> create(boolean defaultValue) {
			try {
				Method createGameruleMethod = ObfuscationReflectionHelper.findMethod(GameRules.BooleanValue.class, "func_82764_b", boolean.class);
				createGameruleMethod.setAccessible(true);
