...
public class ${name}Overlay extends ${JavaModName}Elements.ModElement{
	...
	public void eventHandler(RenderGameOverlayEvent event) {
		...
		if (!event.isCancelable() && event.getType() == RenderGameOverlayEvent.ElementType.HELMET) {
			...
			<#if hasTextures>
				G1StateManager.disableDepthTest();
      			G1StateManager.depthMask(false);
      			G1StateManager.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
      			G1StateManager.color4f(1.0F, 1.0F, 1.0F, 1.0F);
      			G1StateManager.disableAlphaTest();
			</#if>
			...
			<#if hasTextures>
				G1StateManager.depthMask(true);
      			G1StateManager.enableDepthTest();
      			G1StateManager.enableAlphaTest();
      			G1StateManager.color4f(1.0F, 1.0F, 1.0F, 1.0F);
			</#if>
