...
public class ${name}TreeGrower extends <#if (data.megaTrees[0]?has_content) || (data.megaTrees[1]?has_content)>Big</#if>Tree {
   @Nullable @Override protected AbstractTreeFeature<NoFeatureConfig> getTreeFeature(Random randomSource) {
   ...
   @Nullable @Override protected AbstractTreeFeature<NoFeatureConfig> getBigTreeFeature(Random randomSource) {
