...
@Mod.EventBusSubscriber public class ${name}Feature {
	...
	@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD) private static class FeatureRegisterHandler {
		...
		<#if data.plantType == "normal">
			...
			<#else>
				GrassFeature feature = new GrassFeature(GrassFeatureConfig::deserialize) {
					...
					@Override public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos,<#if data.staticPlantGenerationType == "Flower"> NoFeatureConfig config<#else> GrassFeatureConfig config</#if>) {
		...
		<#elseif data.plantType == "double">
			@SubscribeEvent public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			<#if data.doublePlantGenerationType == "Flower">DoublePlantFeature feature = new DoublePlantFeature(DoublePlantConfig::deserialize)<#else>Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize)</#if> {
		        	@Override public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos,<#if data.doublePlantGenerationType == "Flower"> DoublePlant<#else> NoFeature</#if>Config config) {
                			DimensionType dimensionType = world.getDimension().getType();
                			boolean dimensionCriteria = false;
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			biome.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Biome.createDecoratedFeature(feature, new <#if data.plantType == "normal">GrassFeatureConfig(block.getDefaultState())<#else>NoFeatureConfig()</#if>, Placement.NOISE_HEIGHTMAP_32, new NoiseDependant(-0.8, 0, ${data.frequencyOnChunks})));
			<#else>
			biome.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Biome.createDecoratedFeature(feature, <#if data.plantType == "double">new DoublePlantConfig(block.getDefaultState())<#else>IFeatureConfig.NO_FEATURE_CONFIG</#if>, Placement.<#if data.plantType == "normal" || data.plantType == "double">COUNT_HEIGHTMAP_32<#else>COUNT_HEIGHTMAP_DOUBLE</#if>, new FrequencyConfig(${data.frequencyOnChunks})));
			</#if>
		}
	}
