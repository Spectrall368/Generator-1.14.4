...
<#elseif data.mobModelName == "Ocelot">
	<#assign super = "super(context, new OcelotModel(0), " + data.modelShadowSize + "f);">
...
<#elseif data.mobModelName == "Slime">
	<#assign super = "super(context, new SlimeModel(0), " + data.modelShadowSize + "f);">
...
<#elseif data.mobModelName == "Villager">
	<#assign super = "super(context, new VillagerModel(0), " + data.modelShadowSize + "f);">
...
<#else>
	<#assign super = "super(context, new BipedModel(), " + data.modelShadowSize + "f);">
	<#assign model = "BipedModel">
...
public class ${name}Renderer extends <#if humanoid>Biped<#else>Mob</#if>Renderer<${name}Entity, ${model}> {
	...
	<#if data.mobModelGlowTexture?has_content>
	@OnlyIn(Dist.CLIENT) private class GlowingLayer<T extends Entity, M extends EntityModel<T>> extends LayerRenderer<T, M> {
