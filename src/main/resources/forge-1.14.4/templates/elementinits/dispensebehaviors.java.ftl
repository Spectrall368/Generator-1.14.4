	...
	@SubscribeEvent public static void init(FMLCommonSetupEvent event) {
			...
			new OptionalDispenseBehavior() {
				public ItemStack dispenseStack(IBlockSource blockSource, ItemStack stack) {
					...
					this.successful = <@procedureOBJToConditionCode extension.dispenseSuccessCondition/>;
			...
			DispenserBlock.registerDispenseBehavior(${JavaModName}Items.${entity.getModElement().getRegistryNameUpper()}.get(),
					new ${JavaModName}DispenseBoatBehavior(${JavaModName}Boat.Type.${entity.getModElement().getRegistryNameUpper()}));
			</#list>
	}

	<#if specialentities?size != 0>
	public static class ${JavaModName}DispenseBoatBehavior extends DefaultDispenseItemBehavior {
	    private final DefaultDispenseItemBehavior field_218402_b = new DefaultDispenseItemBehavior();
	    private final ${JavaModName}Boat.Type field_218403_c;

	    public ${JavaModName}DispenseBoatBehavior(${JavaModName}Boat.Type type) {
	        this.field_218403_c = type;
	    }
